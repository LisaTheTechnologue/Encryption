...
public class Database {
	...
	public int insertMail(Mail mail){
		int ret = 0;
		String query = "INSERT INTO " + TB_NAME + " (date, sender, recipient, body) VALUES (?, ?, ?, ?);";
	...
	public Mail retrieveMail(String sender, int id){
		Mail mail = null;
		String query = "SELECT * FROM " + TB_NAME + " WHERE sender = ? AND id = ?;";
	...
	public ArrayList<Mail> retrieveMailList(String sender){
		ArrayList<Mail> list = new ArrayList<Mail>();
		String query = "SELECT * FROM " + TB_NAME + " WHERE sender = ?;";
		try(PreparedStatement selectStmt = conn.prepareStatement(query)
		){
			selectStmt.setString(1, sender);			
			ResultSet rs = selectStmt.executeQuery();
	...
	public int deleteMail(String sender, int id){
		int ret = 0;
		String query = "DELETE FROM " + TB_NAME + " WHERE sender = ? AND id = ?;";
		try(PreparedStatement deleteStmt = conn.prepareStatement(query)
		){
			deleteStmt.setString(1, sender);
